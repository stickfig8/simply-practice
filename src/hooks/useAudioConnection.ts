...
import { getDevices, connectAudio, cleanAudioConnection } from "../utils/audioConnectionUtils";

export function useAudioConnection() {

    const {inputId, channel, channelCount, volume, 
        setDevices, setInputId, setChannel, setChannelCount} = useAudioInputStore();

    const audioCtxRef = useRef<AudioContext | null>(null);
    const streamRef = useRef<MediaStream | null>(null);
    const gainRef = useRef<GainNode | null>(null);

    useEffect(() => { // 초기 세팅(장치 목록 로드 + 장치 목록 변경 대응)
    ...
    useEffect(() => { // 장치, 채널, 채널 갯수 변경 시 오디오 재연결
